...
- **"Chưa nhớ" button** (`onRate('forgot')`): icon `refresh` (outline) + label `Chưa nhớ`. Background `theme.colors.surfaceHigh`, icon/text `theme.colors.primary` — the same neutral pairing `IconButton`'s `tone="ghost"` uses. **Not** `danger`/red — "chưa nhớ" is a normal, expected SRS outcome, not an error (matches Q-FLOW-02 / CT-04's rejection of alarming SM-2-style framing).
...
- **Visual**: `flexDirection: 'row'`, `alignItems: 'center'`, `gap: theme.spacing.sm`, `paddingVertical: theme.spacing.sm`, `paddingHorizontal: theme.spacing.lg`. Background `theme.colors.tertiarySoft` (soft gold — the theme's existing "informational" register, distinct from `surface`/`primary`/`danger`), icon colored `theme.colors.onTertiaryContainer`, text `AppText variant="label"` with `style={{color: theme.colors.onTertiaryContainer}}` (same pattern `Chip.tsx` uses to override `AppText`'s fixed color-token set for a non-standard tone).
